...
try {
  if (process.env.FIREBASE_PROJECT_ID && process.env.FIREBASE_PRIVATE_KEY && !process.env.FIREBASE_PRIVATE_KEY.includes('PLACEHOLDER')) {
    ...
    console.log('⚠️  Firebase not configured - authentication features disabled');
  }
} catch (error) {
  console.warn('⚠️  Firebase initialization failed - authentication features disabled:', error.message);
}
...
const corsOptions = {
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'X-Requested-With', 'Accept', 'x-mode'],
